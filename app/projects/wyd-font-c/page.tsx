...
export const metadata: Metadata = {
  title: 'WYD — Font C: Syne + JetBrains Mono',
  description: 'Typography concept C',
}
...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return (
    <p
      className="text-[0.62rem] font-medium tracking-[0.25em] uppercase mb-5"
      style={{ color: 'var(--color-text-muted)', fontFamily: jetbrains.style.fontFamily }}
    >
      {children}
    </p>
  )
...
function MetaLabel({ children }: { children: React.ReactNode }) {
  return (
    <p
      className="text-[0.6rem] font-medium tracking-[0.2em] uppercase mb-1.5"
      style={{ color: 'var(--color-text-muted)', fontFamily: jetbrains.style.fontFamily }}
    >
      {children}
    </p>
...
function ImagePlaceholder({ label, height = 400 }: { label: string; height?: number }) {
  return (
    ...
    >
      <p style={{ color: '#AAAAAA', fontSize: '0.65rem', fontWeight: 500, letterSpacing: '0.14em', textTransform: 'uppercase', textAlign: 'center', padding: '0 2rem', lineHeight: 1.8, fontFamily: jetbrains.style.fontFamily }}>
        {label}
      </p>
    </div>
  )
...
function Callout({ label, value, body }: { label: string; value?: string; body: string }) {
  return (
    <div className="p-5 rounded-lg border" style={{ backgroundColor: 'var(--color-surface)', borderColor: 'var(--color-border)' }}>
      <p className="text-[0.58rem] font-medium tracking-widest uppercase mb-2" style={{ color: 'var(--color-text-muted)', fontFamily: jetbrains.style.fontFamily }}>{label}</p>
      {value && (
        <p className="font-bold mb-1.5" style={{ fontSize: 'clamp(1.5rem, 3vw, 2rem)', color: 'var(--color-text-primary)', lineHeight: 1, fontFamily: jetbrains.style.fontFamily }}>
          {value}
        </p>
      )}
      <p className="text-xs leading-relaxed" style={{ color: 'var(--color-text-secondary)', fontFamily: manrope.style.fontFamily }}>{body}</p>
    </div>
  )
}

export default function WydFontC() {
  return (
    <>
      <div className="sticky top-14 z-40 border-b hidden sm:block" style={{ backgroundColor: 'rgba(255,255,255,0.88)', backdropFilter: 'blur(8px)', WebkitBackdropFilter: 'blur(8px)', borderColor: 'var(--color-border)' }}>
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 h-11 flex items-center justify-between">
          <Link href="/" className="inline-flex items-center gap-1.5 text-xs transition-opacity hover:opacity-60" style={{ color: 'var(--color-text-muted)', fontFamily: jetbrains.style.fontFamily }}>
          ...
          <p className="text-xs font-medium truncate max-w-sm" style={{ color: 'var(--color-text-secondary)', fontFamily: jetbrains.style.fontFamily }}>WYD — Reimagining the Social Event Lifecycle</p>
      ...
      <div className="bg-teal-50 border-b border-teal-200 py-2 px-4 text-center">
        <p className="text-xs font-medium tracking-widest uppercase text-teal-700" style={{ fontFamily: jetbrains.style.fontFamily }}>Font C — Syne + JetBrains Mono</p>
      </div>

      <main id="main-content" className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8" style={{ fontFamily: manrope.style.fontFamily }}>
        ...
        <section className="pt-12 pb-10 md:pt-16">
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_240px] gap-10 lg:gap-16">
            <div>
              ...
              <h1 className="leading-tight mb-6 max-w-3xl" style={{ color: 'var(--color-text-primary)', fontFamily: syne.style.fontFamily, fontWeight: 700, fontSize: 'clamp(2rem, 5vw, 3.5rem)' }}>
            ...
            <div className="hidden lg:flex flex-col gap-4 pt-1">
              <p className="text-[0.6rem] font-medium tracking-widest uppercase" style={{ color: 'var(--color-text-muted)', fontFamily: jetbrains.style.fontFamily }}>At a glance</p>
              <div className="space-y-4">
                {[{ n: '4 → 1', label: 'apps consolidated' }, { n: '3', label: 'friction points eliminated' }, { n: '48', label: 'survey respondents' }, { n: '10', label: 'usability test participants' }].map((item) => (
                  <div key={item.n} className="flex items-baseline gap-2">
                    <span className="font-bold text-lg" style={{ color: 'var(--color-text-primary)', fontFamily: jetbrains.style.fontFamily }}>{item.n}</span>
        ...
        <section className="pb-12 md:pb-16 max-w-3xl">
          <blockquote className="leading-tight mb-8" style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)', color: 'var(--color-text-primary)', fontFamily: syne.style.fontFamily, fontWeight: 800 }}>
        ...
        <section className="pb-16 md:pb-24">
          ...
          <SplitSection aside={<Callout label="Starting assumption" body="The biggest barrier was discoverability — users couldn't find relevant events. This turned out to be only partially true." />}>
            <h2 className="mb-6" style={{ color: 'var(--color-text-primary)', fontFamily: syne.style.fontFamily, fontWeight: 700, fontSize: 'clamp(1.35rem, 2.5vw, 1.9rem)' }}>The Brief</h2>
        ...
        <section className="pb-16 md:pb-24">
          ...
          <SplitSection aside={
            ...
            <h2 className="mb-6" style={{ color: 'var(--color-text-primary)', fontFamily: syne.style.fontFamily, fontWeight: 700, fontSize: 'clamp(1.35rem, 2.5vw, 1.9rem)' }}>Navigating the &ldquo;Obvious&rdquo; Problem</h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-14">
              {[
                ...
                <div key={m.num} className="p-6 rounded-lg" style={{ backgroundColor: 'var(--color-surface)' }}>
                  <p className="text-[0.62rem] font-medium tracking-[0.15em] mb-3" style={{ color: 'var(--color-text-muted)', fontFamily: jetbrains.style.fontFamily }}>{m.num}</p>
                  <p className="text-sm font-semibold mb-2" style={{ color: 'var(--color-text-primary)', fontFamily: syne.style.fontFamily }}>{m.title}</p>
            ...
            <div className="space-y-3">
              {[{ value: '65%', label: 'explicitly requested better personalisation — buried under irrelevant suggestions.' }, { value: '70%', label: 'found coordinating events with friends difficult and were forced off-platform.' }, { value: '80%', label: 'wanted a way to showcase their event history — a need no platform addressed.' }].map((s) => (
                <div key={s.value} className="p-5 rounded-lg border" style={{ backgroundColor: 'var(--color-surface)', borderColor: 'var(--color-border)' }}>
                  <p className="font-bold mb-2" style={{ fontSize: '2rem', lineHeight: 1, color: 'var(--color-text-primary)', fontFamily: jetbrains.style.fontFamily }}>{s.value}</p>
                  <p className="text-xs leading-relaxed" style={{ color: 'var(--color-text-secondary)' }}>{s.label}</p>
                </div>
              ))}
            </div>
          }>
            <h2 className="mb-6" style={{ color: 'var(--color-text-primary)', fontFamily: syne.style.fontFamily, fontWeight: 700, fontSize: 'clamp(1.35rem, 2.5vw, 1.9rem)' }}>What the Data Actually Said</h2>
            <p className="text-base leading-relaxed mb-8 italic" style={{ color: 'var(--color-text-secondary)' }}>&ldquo;The research reframed the problem entirely.&rdquo;</p>
        ...
        <section className="pb-16 md:pb-24">
          ...
          <h2 className="mb-8" style={{ color: 'var(--color-text-primary)', fontFamily: syne.style.fontFamily, fontWeight: 700, fontSize: 'clamp(1.35rem, 2.5vw, 1.9rem)' }}>Reframed Problem Statement</h2>
        ...
        <section className="pb-16 md:pb-24">
          ...
          <SplitSection aside={<Callout label="Grounding filter" body="Every design decision was tested against: does this reduce the need to go off-platform?" />}>
            <h2 className="mb-6" style={{ color: 'var(--color-text-primary)', fontFamily: syne.style.fontFamily, fontWeight: 700, fontSize: 'clamp(1.35rem, 2.5vw, 1.9rem)' }}>Design Principles I Anchored To</h2>
            ...
            <div className="flex flex-col gap-0 mb-14">
              {[
                ...
                <div key={p.num} className="grid grid-cols-[2rem_1fr] gap-4 border-t py-7" style={{ borderColor: 'var(--color-border)' }}>
                  <p className="text-sm font-medium" style={{ color: 'var(--color-text-muted)', fontFamily: jetbrains.style.fontFamily }}>{p.num}</p>
                  <div>
                    <p className="text-base font-semibold mb-1.5" style={{ color: 'var(--color-text-primary)', fontFamily: syne.style.fontFamily }}>{p.title}</p>
        ...
        <section className="pb-20 md:pb-32">
          ...
          <SplitSection aside={<Callout label="Core lesson" body="Functional problems and emotional problems require different design responses. Don't mistake one for the other." />}>
            <h2 className="mb-8" style={{ color: 'var(--color-text-primary)', fontFamily: syne.style.fontFamily, fontWeight: 700, fontSize: 'clamp(1.35rem, 2.5vw, 1.9rem)' }}>The Bigger Lesson</h2>
